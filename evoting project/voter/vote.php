...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $votes_cast = 0;
    $voter_id   = $_SESSION['voter_id']; // Ensure this is defined
    ...
    while ($pos = $positions_res->fetch_assoc()) {
        ...
        // 1. Check if already voted for this position to prevent errors
        ...
        try {
            if ($pos['running_mate_enabled']) {
                // Verify it's a valid presidential candidate
                $stmt = $conn->prepare(
                    "SELECT candidate_id FROM candidates 
                     WHERE candidate_id=? AND position_id=? AND status='approved' AND is_running_mate=0"
                );
                ...
                // IMPORTANT: We only INSERT ONE ROW for the ticket.
                // The candidate_id stored is the President. 
                // You can find the Running Mate later via a JOIN in your results query.
            ...
            // If a duplicate entry still happens somehow, catch it and move to next position
            if ($e->getCode() == 1062) {
                continue;
            } else {
                throw $e;
    ...
    $votes_cast > 0 ? $msg = '✅ Your votes have been cast successfully!' : $err = 'No new votes were cast.';
}
