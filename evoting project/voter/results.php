...
$positions = $conn->query(
    "SELECT p.position_id, p.position_name, p.running_mate_enabled
     FROM positions p
...
foreach ($positions as &$pos) {
    ...
    if ($pos['running_mate_enabled']) {
        // TICKET results — President + VP paired together
        $pos['candidates'] = $conn->query(
            "SELECT c.candidate_id, c.photo,
                    v.first_name, v.last_name,
                    c2.photo AS rm_photo,
        ...
        $pos['candidates'] = $conn->query(
            "SELECT c.candidate_id, c.photo,
                    v.first_name, v.last_name,
                    NULL AS rm_photo, NULL AS rm_first, NULL AS rm_last,
                    COUNT(vt.vote_id) AS vote_count
             FROM candidates c
             JOIN voters v ON v.voter_id = c.voter_id
             LEFT JOIN votes vt ON vt.candidate_id = c.candidate_id
        ...
        <?php else: ?>
            <?php foreach ($pos['candidates'] as $rank => $c): ?>
                <?php
                    ...
                    if (!empty($c['photo']) && $c['photo'] !== 'default.png' && file_exists('../uploads/candidates/'.$c['photo'])) {
                        $photo_url = '../uploads/candidates/'.$c['photo'];
                    }
                    // Running mate photo
                    $rm_photo_url = '';
                    if (!empty($c['rm_first'])) {
                        $rm_photo_url = 'https://ui-avatars.com/api/?name='.urlencode($c['rm_first'].' '.$c['rm_last']).'&background=1e2d45&color=7c3aed&size=128';
                        if (!empty($c['rm_photo']) && $c['rm_photo'] !== 'default.png' && file_exists('../uploads/candidates/'.$c['rm_photo'])) {
                            $rm_photo_url = '../uploads/candidates/'.$c['rm_photo'];
                        }
                    }
                ?>
                <div class="candidate-result">
                    ...
                    <?php if ($pos['running_mate_enabled'] && !empty($c['rm_first'])): ?>
                        <!-- TICKET display — President + VP side by side -->
                        <div style="display:flex;align-items:center;gap:.5rem;flex-shrink:0;">
                            <div style="text-align:center;">
                                ...
                                <div style="font-size:.62rem;color:var(--accent);font-weight:700;margin-top:.15rem;">PRES</div>
                            </div>
                            <div style="font-size:1rem;color:var(--muted);">🤝</div>
                            <div style="text-align:center;">
                                <img src="<?= $rm_photo_url ?>" class="candidate-photo"
                                     alt="<?= htmlspecialchars($c['rm_first']) ?>"
                                     onerror="this.src='https://ui-avatars.com/api/?name=<?= urlencode($c['rm_first'].' '.$c['rm_last']) ?>&background=1e2d45&color=7c3aed&size=128'"
                                     style="border-color:#7c3aed;">
                                <div style="font-size:.62rem;color:#7c3aed;font-weight:700;margin-top:.15rem;">VP</div>
                            </div>
                        </div>
                        <div class="candidate-info">
                            <div class="candidate-name <?= $is_winner ? 'winner' : '' ?>">
                                🏛️ <?= htmlspecialchars($c['first_name'].' '.$c['last_name']) ?>
                                <?= $is_winner ? '<span class="winner-crown">👑</span>' : '' ?>
                            </div>
                            <div style="font-size:.8rem;color:var(--muted);margin-bottom:.3rem;">
                                🤝 <?= htmlspecialchars($c['rm_first'].' '.$c['rm_last']) ?>
                            </div>
