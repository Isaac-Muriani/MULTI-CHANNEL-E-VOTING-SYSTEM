<?php
// =============================================================
//  VOTER REGISTRATION (PIN Version)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // PIN validation
    if (!preg_match('/^\d{4}$/', $pin)) $errors[] = 'PIN must be exactly 4 digits (numbers only).';
    if ($pin !== $confirm_pin)          $errors[] = 'PINs do not match.';
    ...
    if (empty($errors)) {
        $stmt = $conn->prepare(
            "SELECT voter_id FROM voters WHERE national_id=? OR phone=? OR email=?"
        );
        $stmt->bind_param('sss', $national_id, $phone, $email);
        $stmt->execute();
        $stmt->store_result();
        if ($stmt->num_rows > 0) $errors[] = 'A voter with the same National ID, phone, or email already exists.';
        $stmt->close();
    }
    ...
    if (empty($errors)) {
        ...
        $stmt   = $conn->prepare(
            "INSERT INTO voters (first_name,last_name,gender,date_of_birth,national_id,phone,email,password,status)
             VALUES (?,?,?,?,?,?,?,?,'active')"
        );
        $stmt->bind_param('ssssssss', $first_name,$last_name,$gender,$dob,$national_id,$phone,$email,$hashed);
        $stmt->execute() ? $success = 'Registration successful! You can now log in.' : $errors[] = 'Registration failed. Please try again.';
        $stmt->close();
    }
}
?>
...
<head>
    ...
    <style>
        :root { --bg:#0a0f1e; --surface:#111827; --border:#1e2d45; --accent:#00d4ff; --accent2:#7c3aed; --text:#e2e8f0; --muted:#64748b; --error:#f87171; --success:#34d399; }
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .card { background:var(--surface); border:1px solid var(--border); border-radius:16px; padding:2.5rem; width:100%; max-width:560px; box-shadow:0 25px 60px rgba(0,0,0,0.5); }
        ...
        .alert ul { padding-left:1.2rem; } .alert ul li { margin-bottom:.25rem; }
        ...
        input, select { width:100%; background:var(--bg); border:1px solid var(--border); border-radius:8px; padding:.75rem 1rem; color:var(--text); font-family:'Sora',sans-serif; font-size:.9rem; transition:border-color .2s; outline:none; }
        ...
        .pin-hint { font-size:.75rem; color:var(--muted); margin-top:.4rem; }
        ...
        .btn:hover { opacity:.9; }
        .login-link { text-align:center; margin-top:1.5rem; font-size:.875rem; color:var(--muted); }
...
<div class="card">
    ...
    <?php if (!empty($errors)): ?>
        <div class="alert alert-error"><ul><?php foreach($errors as $e): ?><li><?= $e ?></li><?php endforeach; ?></ul></div>
    <?php endif; ?>
    <?php if ($success): ?>
        <div class="alert alert-success"><?= $success ?> <a href="login.php" style="color:inherit;font-weight:700;">Login here →</a></div>
    <?php endif; ?>
    ...
    <form method="POST">
        ...
        <div class="row">
            <div class="field">
                ...
                <input type="text" name="first_name" value="<?= htmlspecialchars($_POST['first_name'] ?? '') ?>" placeholder="John" required>
            </div>
            <div class="field">
                <label>Last Name</label>
                <input type="text" name="last_name" value="<?= htmlspecialchars($_POST['last_name'] ?? '') ?>" placeholder="Doe" required>
            </div>
        </div>
        <div class="row">
            <div class="field">
        ...
        <div class="field">
            ...
            <input type="text" name="national_id" value="<?= htmlspecialchars($_POST['national_id'] ?? '') ?>" placeholder="e.g. 19900101-12345-00001-2" required>
        </div>
        <div class="row">
            <div class="field">
                <label>Phone Number</label>
                <input type="tel" name="phone" value="<?= htmlspecialchars($_POST['phone'] ?? '') ?>" placeholder="+255 7XX XXX XXX" required>
            </div>
            <div class="field">
        ...
        <div class="row">
            <div class="field">
                ...
                <input type="password" name="pin" id="pin"
                       ...
                       inputmode="numeric"
                       oninput="this.value=this.value.replace(/[^0-9]/g,'');updateDots('pin','dot')">
                <div class="pin-hint">Numbers only — like M-Pesa PIN</div>
            </div>
            <div class="field">
                <label>Confirm PIN</label>
                <div class="pin-dots">
                    <div class="pin-dot" id="cdot1"></div>
                    <div class="pin-dot" id="cdot2"></div>
                    <div class="pin-dot" id="cdot3"></div>
                    <div class="pin-dot" id="cdot4"></div>
                </div>
                <input type="password" name="confirm_pin" id="confirm_pin"
                       class="pin-input" maxlength="4"
                       placeholder="••••" required
                       inputmode="numeric"
                       oninput="this.value=this.value.replace(/[^0-9]/g,'');updateDots('confirm_pin','cdot')">
                <div class="pin-hint">Re-enter your PIN</div>
            </div>
        </div>
